...
p = { 
    ...
    'classifier_args': {'base_estimator': sklearn.tree.DecisionTreeClassifier(max_depth=100), 'max_samples':0.5, 'n_estimators':50, 'bootstrap': False}, 
    'n_negatives': 200, 
    'seed': 42, 
    ...
    for file in files:          
        ...
        if validating: 
            for train, val in KFold(N_full, n_folds=5):
                ...
                clf_val = p['classifier'](**p['classifier_args'])
                clf_val.fit(X_train ,y_train)
                
                predictions_val = clf_val.predict_proba(X_val)[:, 1]
                y_pred = numpy.append(y_pred, predictions_val)
                y_real = numpy.append(y_real, y_val)        
