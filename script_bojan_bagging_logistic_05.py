...
p = {
    ...
    'classifier_args': {'base_estimator': sklearn.linear_model.LogisticRegression(C=5000),
                        'max_samples': 0.5,
                        'n_estimators': 100,
                        'bootstrap': False},
    'n_negatives': 200,
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        sys.stdout.write("%d/%d, id: %s, Cumulative validation: %f\r" % (
            counter, const.N_DRIVERS, const.FEATURIZED_DATA_FILES[counter], roc_auc_score(y_real, y_pred)))
        sys.stdout.flush()
        counter += 1
